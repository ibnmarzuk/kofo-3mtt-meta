export const CAPSTONE_TITLE =
  "Kofa — a bounded first-line WhatsApp assistant for Nigerian service shops";

export const PROBLEM_ONE_LINER =
  "Small Nigerian service businesses still answer the same customer questions by hand on WhatsApp. Kofa cuts that first-line load without handing the customer an unsupervised model.";

export const STATEMENT = `The problem. Small Nigerian service businesses — and the 3MTT fellows who build for them — still spend evenings repeating the same WhatsApp answers: hours, price ranges, how to book, what to bring. The Blueprint courses show GenAI can draft and retrieve. They also show the failure mode: unguarded output, invented figures, and the fantasy of replacing the team. The problem is not “we need an AI company.” It is: reduce repeated first-line replies without handing the customer an unsupervised model.
...
export const WORD_COUNT = STATEMENT.trim().split(/\s+/).length;
