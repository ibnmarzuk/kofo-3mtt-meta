  ...
  return (
    <div className="min-h-dvh bg-paper-2/40">
      ...
      <div className="mx-auto grid max-w-6xl gap-8 px-4 py-8 lg:grid-cols-[minmax(0,1fr)_420px] lg:items-start">
        <aside className="order-2 space-y-6 lg:order-1 animate-enter-fade-up">
          <div>
            ...
            <p className="mt-3 max-w-md text-sm leading-relaxed text-ink-soft">
              ...
              customer. Click any test question below or toggle between auto-response mode and human handoff on the phone simulator.
            </p>
          </div>
            ...
            {DEMO_CHIPS.map((c, i) => (
              <li key={c.id}>
                <Link
                  to="/line"
                  search={{ q: c.text }}
                  className="flex gap-3 rounded-lg border border-transparent p-2.5 transition-all duration-150 hover:border-line hover:bg-paper-3/80 hover:translate-x-1"
                >
        ...
        <div className="order-1 lg:order-2">
          {hydrated ? (
            ...
            <div className="mx-auto h-[min(760px,calc(100dvh-8rem))] max-w-[420px] rounded-[2rem] bg-ink/90 animate-pulse" />
          )}
        </div>
      </div>
    </div>
